...
fn hex_decode_fails_on_invalid() {
    ...
    assert!(decode("ggg") == None);
    assert!(decode("gsdfkhg") == None);
    assert!(decode("ffffffx") == None);
}
...
fn hex_decode_works_on_valid() {
    ...
    assert!(decode("0001020304").unwrap() == [0, 1, 2, 3, 4]);
    assert!(decode("FF").unwrap() == [255]);
    assert!(decode("90AF").unwrap() == [144, 175]);
    assert!(decode("deadbeef").unwrap() == [222, 173, 190, 239]);
}
...
fn hex_decode_works_on_odd() {
    ...
    // a zero)
    assert!(decode("A").unwrap() == [10]);
    assert!(decode("012").unwrap() == [0, 18]);
    assert!(decode("eadbeef").unwrap() == [14, 173, 190, 239]);
}

pub fn encode(input: &[u8], uppercase: bool) -> String {
...
fn hex_encode_works_correctly_with_ascii() {
    assert_eq!(encode("abcdefg".as_bytes(), true), "61626364656667");
    assert_eq!(encode("abcdefg".as_bytes(), false), "61626364656667");
    assert_eq!(encode("JKLMNOP".as_bytes(), false), "4a4b4c4d4e4f50");
    assert_eq!(encode("JKLMNOP".as_bytes(), true), "4A4B4C4D4E4F50");
}

#[test]
fn hex_encode_works_correctly_with_binary() {
    assert_eq!(encode(&[0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF], true), "1234567890ABCDEF");
    assert_eq!(encode(&[0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF], false), "1234567890abcdef");
}

#[test]
fn hex_reencode_works_correctly() {
    assert_eq!(decode(&encode("DeadBeefCafeBabe".as_bytes(), true)).unwrap(), "DeadBeefCafeBabe".as_bytes());
    assert_eq!(decode(&encode("DeadBeefCafeBabe".as_bytes(), false)).unwrap(), "DeadBeefCafeBabe".as_bytes());
}
